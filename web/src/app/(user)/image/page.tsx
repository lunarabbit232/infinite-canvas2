...
import { interrogateImage } from "@/services/api/assets";

import type { GeneratedImage, GenerationCategory, GenerationLog, GenerationLogConfig, GenerationResult, RequestSnapshot, ResultViewMode, UpdateAiConfig, WorkbenchLayout } from "./types";
import { CATEGORY_STORE_KEY, buildGenerationLogConfig, buildLog, categoryStore, clampWorkflowButtonPosition, createPendingResult, createWorkflowResultId, dedupeGenerationLogs, defaultWorkflowButtonPosition, disposableLogStorageKeys, errorDetail, errorMessage, formatLogTime, imageExtension, imageLogFromTask, imageLogIdentityKeys, imageLogTaskId, imageResultIdentityKeys, imageResultMatchesLog, imageTaskChannelId, imageTaskIdentityKeys, isRecoverableImageTask, logStore, mergeBackendImageTasks, mergeGenerationLogs, mergePendingLogResults, readStoredCategories, readStoredLogs, replaceStoredImageHistory, resolveImageChannelId, serializeLog, shouldDeleteReferenceFile, updateResult, updateResultByLogId, withWorkflowLogCategories } from "./utils/image-log-helpers";
...
export default function ImagePage() {
    ...
    const interrogateInputRef = useRef<HTMLInputElement>(null);
    ...
    }, [isUserReady, token, syncBackendImageTasks]);
    ...
    const handleInterrogateImage = () => {
        interrogateInputRef.current?.click();
    };

    const onInterrogateFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
        const file = e.target.files?.[0];
        if (!file) return;
        try {
            message.loading({ content: "AI 正在分析图片…", key: "interrogate" });
            const promptText = await interrogateImage(file);
            setPrompt(promptText);
            message.success({ content: "已生成提示词", key: "interrogate" });
        } catch (err: any) {
            message.error({ content: err.message || "反推失败", key: "interrogate" });
        } finally {
    ...
    return (
        <div className="flex h-full flex-col overflow-hidden bg-stone-50 text-stone-900 dark:bg-stone-950 dark:text-stone-100">
            {sourceCanvas ? <div className="shrink-0 bg-violet-50 px-4 py-1.5 text-xs text-violet-600 dark:bg-violet-950 dark:text-violet-300">← 从画布跳转而来，生成后可回填到画布节点</div> : null}
                ...
                {workbenchLayout === "side" ? (
                    <>
                        <WorkbenchPanel
                            ...
                            onInterrogateImage={handleInterrogateImage}
                        />
                        <ResultsPanel
                            results={results}
                            logs={logs}
                    ...
                    <div className="flex flex-col gap-3">
                        <div className="grid gap-2 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-start">
                            ...
                            <div className="flex shrink-0 flex-wrap items-center justify-end gap-2">
                                ...
                                {onInterrogateImage ? <Button title="AI反推提示词" icon={<WandSparkles className="size-4" />} onClick={onInterrogateImage}>AI</Button> : null}
                                <Button
                                    title="参数配置"
                ...
                <section className="overflow-hidden rounded-lg border border-stone-200 bg-background dark:border-stone-800">
                    ...
                    <div className="border-t border-stone-200 p-3 dark:border-stone-800 space-y-2">
                        <div className="flex flex-wrap gap-1">
                            ...
                            {onInterrogateImage ? <Button size="small" icon={<WandSparkles className="size-3.5" />} onClick={onInterrogateImage}>AI反推</Button> : null}
                        </div>
