import { ArrowLeft, ArrowRight, Maximize2, Plus, BookOpen, ClipboardPaste, FolderPlus, Music2, PanelBottom, PanelLeft, Sparkles, Trash2, Upload, VideoIcon } from "lucide-react";
import { App, Button, Input, Switch, Tag } from "antd";
import { useEffect, useRef, useState, type ReactNode } from "react";

import { ModelPicker } from "@/components/model-picker";
import { KlingStoryboardPanel } from "./kling-storyboard-panel";
...
type WorkbenchLayout = "side" | "bottom" | "fullscreen";
        ...
        <div className="flex min-h-[420px] flex-col overflow-hidden rounded-lg border border-stone-200 bg-card shadow-sm dark:border-stone-800 lg:min-h-0">
            ...
            <div className="thin-scrollbar min-h-0 flex-1 space-y-3 overflow-y-auto px-4 pb-3">
                    ...
                    <>
                        {references.length > 0 ? (
                            <div className="flex justify-end">
                                <Button size="small" icon={<Sparkles className="size-3.5" />} onClick={() => {
                                    const elements = references.slice(0, 3).map((ref, i) => ({ name: `元素${i + 1}`, description: ref.name, references: [{ id: ref.id, kind: "image" as const, name: ref.name, type: ref.type, dataUrl: ref.dataUrl, storageKey: ref.storageKey }] }));
                                    const prompts = references.slice(0, elements.length).map((_, i) => ({ prompt: `第${i + 1}帧`, duration: "1" }));
                                    updateElementList(elements);
                                    if (prompts.length) updateConfig("videoMultiPrompt", prompts);
                                }}>自动填充参考图到元素列表</Button>
                            </div>
                        ) : null}
                ...
                {multiShot && (isKIEKlingV3 || shotType === "customize") ? (
                    <KlingSection title={TEXT.shotPrompt} count={multiPrompts.length}>
                        <KlingStoryboardPanel
                            items={multiPrompts}
                            selectedIndex={selectedShotIndex}
                            onSelectIndex={setSelectedShotIndex}
                            onChange={(next) => updateConfig("videoMultiPrompt", next)}
                            onAdd={addMultiPrompt}
                            onRemove={removeMultiPrompt}
                        />
                    </KlingSection>
                ) : null}
