...
import { isCanvasImageNodeType, isPanoramaNodeType, PANORAMA_IMAGE_SIZE } from "./canvas-panorama";
import type { CanvasConnection, CanvasNodeData, CanvasNodeMetadata } from "../types";
import { CanvasNodeType } from "../types";

// ============================================================
// 常量（从 canvas-client-page.tsx 迁出）
// ============================================================
...
export function applyCanvasVideoTaskUpdate(
...
export function applyCanvasImageTaskUpdate(
    nodes: CanvasNodeData[],
    nodeId: string,
    ...
    return nodes.map((node) => {
        ...
        if (!completed || !url) return { ...node, metadata };
        const naturalWidth = task.naturalWidth ?? task.width ?? fallbackSize.width;
        const naturalHeight = task.naturalHeight ?? task.height ?? fallbackSize.height;
        const isPanorama = isPanoramaNodeType(node.type) || (node.type === CanvasNodeType.Image && naturalWidth / (naturalHeight || 1) >= 1.9);
        const imageSize = fitNodeSize(naturalWidth, naturalHeight, node.type === CanvasNodeType.Panorama ? Infinity : node.metadata?.nodeMaxWidth || Infinity, node.metadata?.nodeMaxHeight || Infinity);
        return {
            ...node,
...
export function applyCanvasAudioTaskUpdate(
    nodes: CanvasNodeData[],
    nodeId: string,
    task: CanvasAudioTask,
    startedAt: number,
) {
    return nodes.map((node) => {
        if (node.id !== nodeId) return node;
...
export function getInputSummary(inputs: { type: string }[]) {
    return {
...
import type { CanvasImageGenerationType } from "../types";
...
export function buildImageGenerationMetadata(type: CanvasImageGenerationType, config: AiConfig, count: number, references: ReferenceImage[]): CanvasNodeMetadata {
    return {
        ...
        count,
        references: references.map(referenceUrlHelper).filter((url): url is string => Boolean(url)),
    };
}
...
function referenceUrlHelper(image: ReferenceImage) {
...
export function generationReferenceUrls(context: { referenceImages: ReferenceImage[]; firstFrame?: ReferenceImage | null; lastFrame?: ReferenceImage | null; referenceVideos: Array<{ storageKey?: string; url?: string }>; referenceAudios?: Array<{ storageKey?: string; url?: string }> }) {
    return [
        context.firstFrame ? referenceUrlHelper(context.firstFrame) : null,
        context.lastFrame ? referenceUrlHelper(context.lastFrame) : null,
        ...context.referenceImages.map(referenceUrlHelper).filter((url): url is string => Boolean(url)),
...
export function getConnectionTargetAnchor(node: CanvasNodeData, current: { handleType: string }) {
    return {
