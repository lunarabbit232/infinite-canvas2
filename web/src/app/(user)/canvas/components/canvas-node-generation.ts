...
export function buildNodeGenerationInputs(nodeId: string, nodes: CanvasNodeData[], connections: CanvasConnection[]): NodeGenerationInput[] {
    return getGenerationResourceNodes(nodeId, nodes, connections).flatMap((node): NodeGenerationInput[] => {
        ...
        const charInputs = readCharacterInputs(node);
        if (charInputs.length) return charInputs;
...
function readCharacterInputs(node: CanvasNodeData): NodeGenerationInput[] {
    if (node.type !== CanvasNodeType.Character || !node.metadata?.characterId) return [];
    const results: NodeGenerationInput[] = [];
    const template = node.metadata.characterPromptTemplate || "";
    if (template) {
        results.push({ nodeId: node.id, type: "text", title: node.title, text: `${node.title}：${template}` });
    }
    const urls: string[] = node.metadata.characterReferenceUrls || [];
    urls.forEach((url, index) => {
        results.push({
            nodeId: node.id,
            type: "image",
            title: `${node.title}-参考${index + 1}`,
            image: { id: `${node.id}-ref-${index}`, name: `${node.title}-${index + 1}.png`, type: "image/png", dataUrl: url, url },
        });
    });
    return results;
}
