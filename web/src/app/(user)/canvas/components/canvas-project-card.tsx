...
import { Archive, Check, Download, Pencil, Trash2, Undo2, X } from "lucide-react";
...
export function CanvasProjectCard({ project }: { project: CanvasProject }) {
    ...
    const archiveProject = useCanvasStore((state) => state.archiveProject);
    const unarchiveProject = useCanvasStore((state) => state.unarchiveProject);
                ...
                <div className="flex items-center gap-1" onClick={(event) => event.stopPropagation()}>
                    {editing ? (
                        ...
                        <>
                            ...
                            <Button type="text" size="small" shape="circle" icon={isArchived ? <Undo2 className="size-4" /> : <Archive className="size-4" />} onClick={() => isArchived ? unarchiveProject(project.id) : archiveProject(project.id)} aria-label={isArchived ? "取消归档" : "归档"} />
