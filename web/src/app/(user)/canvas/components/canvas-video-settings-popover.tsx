        ...
        <>
            ...
            {imageOptions.length > 0 ? (
                <div className="flex justify-end">
                    <button type="button" className="h-8 rounded-full border px-3 text-xs transition hover:opacity-80" style={{ borderColor: theme.node.stroke, color: theme.node.text }}
                        onClick={() => {
                            const elements = imageOptions.slice(0, 3).map((opt, i) => ({ name: `元素${i + 1}`, description: opt.label, nodeIds: [opt.nodeId] }));
                            if (elements.length) updateElementList(elements);
                        }}>
                        + 自动填充图片
                    </button>
                </div>
