...
import { Download, Ellipsis, FolderPlus, Image as ImageIcon, Info, MessageSquare, Minus, Music2, Pencil, Plus, RefreshCw, Scissors, Settings2, Trash2, Upload, Video } from "lucide-react";
...
type CanvasNodeHoverToolbarProps = {
    ...
    onClipVideo: (node: CanvasNodeData) => void;
};
...
export function CanvasNodeHoverToolbar({
    ...
    const nodeToolbarTools: ToolbarTool[] = [
        ...
        ...(isVideo && hasVideo ? [{ id: "clipVideo", title: "截取片段", label: "截取", icon: <Scissors className="size-4" />, onClick: () => onClipVideo(node) }] : []),
...
export function CanvasNodeInfoModal({ node, open, onClose, characterRefUrls, compareTargetUrl }: { node: CanvasNodeData | null; open: boolean; onClose: () => void; characterRefUrls?: string[]; compareTargetUrl?: string }) {
                ...
                <div className="h-[56vh] min-h-[360px] text-sm">
                    {view === "info" ? (
                        <div className="thin-scrollbar h-full space-y-3 overflow-auto pr-1">
                            ...
                            {node.type === CanvasNodeType.Character && characterRefUrls?.length && compareTargetUrl ? (
                                <div className="rounded-lg border p-3" style={{ borderColor: theme.node.stroke }}>
                                    {characterRefUrls.map((url, i) => (
                                        <FaceSimilarityCompare key={i} imageA={url} imageB={compareTargetUrl} label={`参考图${i + 1} 相似度`} />
                                    ))}
                                </div>
                            ) : null}
                        </div>
