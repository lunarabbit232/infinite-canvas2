...
import { ChevronRight, Image as ImageIcon, Music2, RefreshCw, Star, UserRound, Video } from "lucide-react";

import { canvasThemes } from "@/lib/canvas-theme";
import { formatBytes, formatDuration } from "@/lib/image-utils";
import { useThemeStore } from "@/stores/use-theme-store";
import { getVideoThumbnail } from "@/services/file-storage";
import { CanvasResourceMentionTextarea } from "./canvas-resource-mention-textarea";
import { CanvasNodeType, type CanvasNodeData, type Position } from "../types";
import { isCanvasImageNodeType } from "../utils/canvas-panorama";
import type { CanvasResourceReference } from "../utils/canvas-resource-references";
import { getNodeTypeColor } from "../utils/canvas-node-colors";
...
export const CanvasNode = React.memo(function CanvasNode({
    ...
    return (
        ...
        >
            {!isGroup ? (
                <div
                    className="absolute left-0 top-0 z-[66] h-[3px] w-full rounded-t-[inherit]"
                    style={{ backgroundColor: nodeColor.hex }}
                    title={`${nodeColor.label}节点`}
                />
            ) : null}
            <div
...
function VideoNodeContent({ node, theme }: NodeContentRendererProps) {
    ...
    useEffect(() => {
        if (posterSetRef.current || !node.metadata?.content) return;
        posterSetRef.current = true;
        getVideoThumbnail(node.metadata.content).then((u) => { if (u) setPoster(u); }).catch(() => {});
    ...
    return <video src={node.metadata.content} controls className="h-full w-full rounded-[18px] bg-black object-contain" data-canvas-no-zoom data-canvas-node-id={node.id} poster={poster || undefined} />;
}

function CharacterNodeContent({ node, theme }: NodeContentRendererProps) {
    const name = node.metadata?.characterName || "未选择角色";
    const coverUrl = node.metadata?.content;
    const hasRefs = Boolean((node.metadata?.characterReferenceUrls as string[] | undefined)?.length);
    const handleCompare = () => { window.dispatchEvent(new CustomEvent("canvas:face-compare", { detail: { nodeId: node.id } })); };
    if (coverUrl)
        return (
            <div className="flex h-full w-full flex-col overflow-hidden rounded-[18px]">
                <img src={coverUrl} alt={name} className="h-0 flex-1 w-full object-cover" />
                <div className="flex h-10 items-center gap-2 px-3 text-sm font-medium" style={{ background: theme.node.panel, color: theme.node.text }}>
                    <UserRound className="size-4" />
                    <span className="truncate">{name}</span>
                    {hasRefs ? (
                        <button type="button" className="ml-auto grid size-6 shrink-0 place-items-center rounded-full text-xs transition hover:opacity-70" style={{ background: theme.node.fill }} title="面部比对" onClick={(e) => { e.stopPropagation(); handleCompare(); }}>👤</button>
                    ) : null}
                </div>
            </div>
        );
    return (
        <div className="flex h-full w-full flex-col items-center justify-center gap-3" style={{ color: theme.node.placeholder }}>
            <UserRound className="size-7 opacity-35" />
            <span className="text-sm">{name}</span>
            {hasRefs ? (
                <button type="button" className="rounded-md border px-2 py-1 text-xs transition hover:opacity-70" style={{ borderColor: theme.node.stroke }} onClick={(e) => { e.stopPropagation(); handleCompare(); }}>面部比对</button>
            ) : null}
        </div>
    );
}
