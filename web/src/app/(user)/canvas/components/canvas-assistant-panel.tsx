...
import { ArrowUp, Clapperboard, History, ImageIcon, LoaderCircle, MessageSquare, PanelRightClose, Plus, RotateCcw, Settings2, Sparkles, Trash2, X } from "lucide-react";
...
type CanvasAssistantPanelProps = {
    ...
    onCollapse: () => void;
    onApplyStoryboard?: (scenes: { scene: number; location?: string; shots: { shot: number; prompt: string }[] }[]) => void;
};

export function CanvasAssistantPanel({ nodes, selectedNodeIds, sessions, activeSessionId, onSelectNodeIds, onSessionsChange, onInsertImage, onInsertText, onPasteImage, onCollapseStart, onCollapse, onApplyStoryboard }: CanvasAssistantPanelProps) {
    ...
    return (
        ...
        >
            ...
            >
                ...
                <div className="flex items-center justify-between border-b px-4 py-3" style={{ borderColor: theme.node.stroke }}>
                    ...
                    <div className="flex items-center gap-1">
                        <Tooltip title="导演 Agent">
                            <Button type="text" shape="circle" className="!h-8 !w-8 !min-w-8" style={iconButtonStyle} icon={<Clapperboard className="size-4" />} onClick={() => setDirectorOpen(true)} />
                        </Tooltip>
                        <Tooltip title="编剧 Agent">
                            <Button type="text" shape="circle" className="!h-8 !w-8 !min-w-8" style={iconButtonStyle} icon={<MessageSquare className="size-4" />} onClick={() => setStoryboardOpen(true)} />
                        </Tooltip>
                        {view === "history" ? (
                            <>
                                <Tooltip title="删除选中">
            ...
            <StoryboardAgentModal open={storyboardOpen} onClose={() => setStoryboardOpen(false)} onApplyScenes={onApplyStoryboard} />
