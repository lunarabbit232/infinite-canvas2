...
import { BookOpen, ChevronRight, Clapperboard, Eye, FileText, Group, Image as ImageIcon, Music2, Plus, Search, Settings2, Type, UserRound, Video } from "lucide-react";
...
const NODE_TYPE_ICON = {
    ...
    [CanvasNodeType.Character]: UserRound,
};
    ...
    return (
        ...
        >
            <motion.aside
                ...
                data-canvas-no-zoom
            >
                <div className="flex items-center gap-0.5 border-b px-2 py-2" style={{ borderColor: theme.toolbar.border }}>
                    <NavLink href="/canvas" icon={<ImageIcon className="size-4" />} label="画布" theme={theme} />
                    <NavLink href="/image" icon={<span className="text-sm">🖼</span>} label="生图" theme={theme} />
                    <NavLink href="/video" icon={<Video className="size-4" />} label="视频" theme={theme} />
                    <NavLink href="/prompts" icon={<BookOpen className="size-4" />} label="提示词" theme={theme} />
                    <NavLink href="/assets" icon={<span className="text-sm">📦</span>} label="素材" theme={theme} />
                </div>
...
function NavLink({ href, icon, label, theme }: { href: string; icon: React.ReactNode; label: string; theme: CanvasTheme }) {
    return (
        <Link
            href={href}
            title={label}
            className="flex h-8 w-8 items-center justify-center rounded-lg opacity-55 transition hover:bg-black/5 hover:opacity-100 dark:hover:bg-white/10"
    ...
    const filtered = useMemo(() => {
        ...
        return assets.filter(
            (asset) =>
                ...
                (!query || [asset.title, ...(asset.tags || [])].join(" ").toLowerCase().includes(query)),
        );
    }, [assets, keyword, type, tagFilter]);
        ...
        <>
            ...
            {tags.length > 0 ? (
                <div className="flex flex-wrap gap-1 px-3 pb-2">
                    <TagButton label="全部" active={!tagFilter} theme={theme} onClick={() => setTagFilter("")} />
                    {tags.map((tag) => (
                        <TagButton key={tag} label={tag} active={tagFilter === tag} theme={theme} onClick={() => setTagFilter(tagFilter === tag ? "" : tag)} />
                    ))}
                </div>
            ) : null}
...
function TagButton({ label, active, theme, onClick }: { label: string; active: boolean; theme: CanvasTheme; onClick: () => void }) {
    return (
        <button
            type="button"
            onClick={onClick}
