        ...
        <Card
            ...
            cover={
                <button type="button" className="block w-full text-left" onClick={onOpen}>
                    {item.coverUrl ? (
                        <Image src={item.coverUrl} alt={item.title} width={400} height={300} unoptimized className="aspect-[4/3] w-full object-cover" />
                    ) : (
                        <div className="flex aspect-[4/3] items-center justify-center bg-stone-100 dark:bg-stone-800">
                            <span className="text-xs text-stone-400 dark:text-stone-500">{item.title.slice(0, 8)}</span>
                        </div>
                    )}
                </button>
            }
