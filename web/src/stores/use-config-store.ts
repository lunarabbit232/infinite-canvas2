...
export const defaultConfig: AiConfig = {
    ...
    audioVoice: "FunAudioLLM/CosyVoice2-0.5B:alex",
    audioFormat: "mp3",
    audioSpeed: "1",
    audioInstructions: "",
    videoSeconds: "6",
...
function resolveEffectiveConfig(config: AiConfig, modelChannel: AdminPublicSettings["modelChannel"] | null, canUseRemoteChannel: boolean) {
    ...
    const nextModel = textModels.includes(config.model) ? config.model : fallbackModel;
    const nextImageModel = imageModels.includes(config.imageModel) ? config.imageModel : fallbackImageModel;
    const nextVideoModel = videoModels.includes(config.videoModel) ? config.videoModel : fallbackVideoModel;
    const nextTextModel = textModels.includes(config.textModel) ? config.textModel : fallbackTextModel || fallbackModel;
    const nextAudioModel = audioModels.includes(config.audioModel) ? config.audioModel : fallbackAudioModel;
    return {
        ...config,
        channelMode,
        models,
        ...
        // 模型切换（尤其切云端渠道后默认模型变化）时同步对应渠道 ID，避免残留旧渠道（如 Vidu）导致请求带错 X-Model-Channel-ID。
        imageChannelId: resolveChannelIdForModel(channels, nextImageModel, config.imageChannelId),
        videoChannelId: resolveChannelIdForModel(channels, nextVideoModel, config.videoChannelId),
        textChannelId: resolveChannelIdForModel(channels, nextTextModel, config.textChannelId),
        audioChannelId: resolveChannelIdForModel(channels, nextAudioModel, config.audioChannelId),
        systemPrompt: modelChannel.systemPrompt,
        publicChannels: channels,
    };
}

function resolveChannelIdForModel(channels: Array<{ id?: string; models?: string[] }>, model: string, fallbackId: string) {
