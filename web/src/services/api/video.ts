...
export type VideoTaskCreateOptions = { clientTaskId?: string; source?: "video-workbench" | "canvas" | "workflow"; sourceId?: string; characterIds?: string[] };
...
export async function createVideoGenerationTask(config: AiConfig, prompt: string, references: ReferenceImage[] | VideoReferenceInput = [], onProgress?: VideoProgressHandler, options?: string | VideoTaskCreateOptions): Promise<CreatedVideoGenerationTask> {
    ...
    if (isViduVideoModel(model)) {
        const input = normalizeVideoReferenceInput(references);
        return createViduTask(config, model, prompt, input.references, options);
    }
    ...
    try {
        ...
        const headers = { ...aiHeaders(config), ...(accountProxy && createOptions.clientTaskId ? { "X-Client-Video-Task-ID": createOptions.clientTaskId } : {}), ...(accountProxy && createOptions.source ? { "X-Video-Task-Source": createOptions.source } : {}), ...(accountProxy && createOptions.sourceId ? { "X-Video-Task-Source-ID": createOptions.sourceId } : {}), ...(accountProxy && createOptions.characterIds?.length ? { "X-Character-IDs": createOptions.characterIds.join(",") } : {}) };
...
export async function pollVideoGenerationTaskStatus(config: AiConfig, task: VideoResponse) {
    const model = config.model || config.videoModel;
    if (isViduVideoModel(model)) return pollViduTaskStatus(config, task);
    const pollId = videoPollId(model, task);
    if (!pollId) throw new VideoRequestError("视频接口没有返回任务 ID", task);
...
export async function cancelVideoGenerationTask(config: AiConfig, task?: VideoResponse | null) {
    ...
    const id = task.id || task.task_id || task.video_id;
    if (!id) return;
    await axios.post(`/api/v1/video-tasks/${encodeURIComponent(id)}/cancel`, null, { headers: aiHeaders(config) });
}
...
function isViduVideoModel(model: string) {
    return model.startsWith("viduq");
}

async function createViduTask(config: AiConfig, model: string, prompt: string, references: ReferenceImage[], options?: string | VideoTaskCreateOptions): Promise<CreatedVideoGenerationTask> {
    const createOptions = normalizeVideoTaskCreateOptions(options);
    const channel = localChannelForActiveModel(config);
    const apiKey = channel?.apiKey || config.apiKey;
    const baseUrl = (channel?.baseUrl || config.baseUrl).trim().replace(/\/+$/, "").replace(/\/ent\/v2\/?$/, "");
    const body: Record<string, unknown> = {
        model,
        prompt,
        images: references.map((ref) => ref.url),
        duration: viduDuration(config.videoSeconds, model),
        resolution: viduResolution(config.vquality),
        aspect_ratio: viduAspectRatio(config.size),
    };
    const startedAt = Date.now();
    try {
        const accountProxy = usesAccountProxy(config);
        const responseData = accountProxy
            ? (await axios.post("/api/v1/videos", body, { headers: { "Content-Type": "application/json", ...aiHeaders(config), ...(createOptions.clientTaskId ? { "X-Client-Video-Task-ID": createOptions.clientTaskId } : {}), ...(createOptions.source ? { "X-Video-Task-Source": createOptions.source } : {}), ...(createOptions.sourceId ? { "X-Video-Task-Source-ID": createOptions.sourceId } : {}), ...(createOptions.characterIds?.length ? { "X-Character-IDs": createOptions.characterIds.join(",") } : {}) } })).data
            : (await axios.post(`${baseUrl}/ent/v2/reference2video`, body, { headers: { Authorization: `Token ${apiKey}`, "Content-Type": "application/json" } })).data;
        const task = accountProxy ? unwrapVideoResponse(responseData) : normalizeVideoResponse(responseData);
        if (!task.id) throw new Error("Vidu 没有返回任务 ID");
        return { task, pollId: task.id, startedAt, requestBody: body };
    } catch (error) {
        const { message, detail } = readAxiosError(error, "Vidu 视频生成失败");
        throw new VideoRequestError(message, detail);
    }
}

async function pollViduTaskStatus(config: AiConfig, task: VideoResponse): Promise<VideoResponse> {
    const pollId = task.id || task.task_id || "";
    if (!pollId) throw new VideoRequestError("Vidu 任务 ID 为空", task);
    const accountProxy = usesAccountProxy(config);
    if (accountProxy) {
        const response = await axios.get(`/api/v1/videos/${encodeURIComponent(pollId)}`, { headers: aiHeaders(config) });
        return unwrapVideoResponse(response.data);
    }
    const channel = localChannelForActiveModel(config);
    const apiKey = channel?.apiKey || config.apiKey;
    const baseUrl = (channel?.baseUrl || config.baseUrl).trim().replace(/\/+$/, "").replace(/\/ent\/v2\/?$/, "");
    const response = await axios.get(`${baseUrl}/ent/v2/tasks/${encodeURIComponent(pollId)}`, {
        headers: { Authorization: `Token ${apiKey}` },
    });
    return normalizeVideoResponse(response.data);
}

function viduDuration(seconds: string, model: string) {
    const s = Math.floor(Math.max(1, Math.min(16, Number(seconds) || 5)));
    if (model.startsWith("viduq3")) return Math.max(3, Math.min(16, s));
    if (model.startsWith("viduq2")) return Math.max(1, Math.min(10, s));
    if (model.startsWith("viduq1")) return 5;
    return Math.min(s, 8);
}

function viduResolution(vquality: string) {
    if (vquality === "540p") return "540p";
    if (vquality === "1080p") return "1080p";
    return "720p";
}

function viduAspectRatio(size: string) {
    const ratios: Record<string, string> = { "1024x1024": "1:1", "768x1024": "3:4", "1024x768": "4:3", "1280x720": "16:9", "720x1280": "9:16" };
    return ratios[size] || "16:9";
}

export function clipVideoByUrl(url: string, start: number, end: number) {
    return apiPost<{ url: string }>("/api/v1/videos/clip-by-url", { url, start, end }, useUserStore.getState().token);
}

export function concatVideoByUrls(urls: string[]) {
    return apiPost<{ url: string }>("/api/v1/videos/concat-by-url", { urls }, useUserStore.getState().token);
}

export function transitionVideoByUrls(urls: string[], transition: string, duration: number) {
    return apiPost<{ url: string }>("/api/v1/videos/transition-by-url", { urls, transition, duration }, useUserStore.getState().token);
}
