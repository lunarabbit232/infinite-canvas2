...
const thumbnailCache = localforage.createInstance({ name: "infinite-canvas", storeName: "video_thumbnails" });

export function getVideoThumbnail(url: string, seekTime = 1): Promise<string> {
    if (!url) return Promise.resolve("");
    return thumbnailCache.getItem<string>(url).then((cached) => {
        if (cached) return cached;
        return captureBestThumbnail(url, [0.5, 1, 2]).then((dataUrl) => {
            if (dataUrl) thumbnailCache.setItem(url, dataUrl).catch(() => {});
            return dataUrl;
        }).catch(() => "");
    });
}

function captureBestThumbnail(url: string, seekTimes: number[]): Promise<string> {
    if (!url) return Promise.resolve("");
    return new Promise<string>((resolve) => {
        ...
        const captureCanvas = () => {
            ...
            const h = video.videoHeight || Math.round(w / 16 * 9);
            const scale = Math.min(1, maxWidth / w);
            const canvas = document.createElement("canvas");
            canvas.width = Math.round(w * scale);
            canvas.height = Math.round(h * scale);
            const ctx = canvas.getContext("2d");
            if (!ctx) return { dataUrl: "", brightness: 0 };
            ctx.drawImage(video, 0, 0, canvas.width, canvas.height);
            const imageData = ctx.getImageData(0, 0, canvas.width, canvas.height);
            const dataUrl = canvas.toDataURL("image/jpeg", 0.7);
            return { dataUrl, brightness: averageBrightness(imageData.data) };
        };

        const duration = Math.max(0, video.duration || 10);
        const times = seekTimes.filter((t) => t < duration);
        if (!times.length) {
            video.onloadeddata = () => done(captureCanvas().dataUrl);
            video.onerror = () => fail();
            video.src = url;
            return;
        }

        const captures: { dataUrl: string; brightness: number }[] = [];
        let timeIndex = 0;

        const seekNext = () => {
            if (timeIndex >= times.length) {
                const best = captures.reduce((a, b) => (a.brightness > b.brightness ? a : b), captures[0]);
                done(best?.dataUrl || "");
                return;
            }
