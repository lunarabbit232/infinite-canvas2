...
func CreateCanvasImageTask(w http.ResponseWriter, r *http.Request) {
	...
	body, contentType, endpoint, source, nodeID, sourceID, clientTaskID, prompt, channelID, characterIDs, err := readCanvasTaskAIRequest(r, "/images/generations")
	if err != nil {
		Fail(w, err.Error())
		return
	}
	...
	if len(characterIDs) > 0 {
		body, prompt = injectCharacterAnchor(user, body, prompt, modelName, characterIDs)
	}
	...
	if err != nil {
		logger.Errorf("canvas image task select channel failed: model=%s err=%v", modelName, err)
	...
	if err != nil {
		logger.Errorf("create canvas image task failed: user=%s err=%v", user.ID, err)
		Fail(w, "AI 接口请求失败")
		return
	}
...
func GetCanvasImageTask(w http.ResponseWriter, r *http.Request, id string) {
	...
	if err != nil {
		logger.Errorf("read canvas image task failed: user=%s id=%s err=%v", user.ID, id, err)
		Fail(w, "AI 接口请求失败")
		return
	}
	if !found {
...
func UserCanvasImageTasks(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		logger.Errorf("list canvas image tasks failed: user=%s err=%v", user.ID, err)
		Fail(w, "AI 接口请求失败")
		return
	}
	OK(w, tasks)
...
func BatchCanvasImageTasks(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		logger.Errorf("batch canvas image tasks failed: user=%s err=%v", user.ID, err)
		Fail(w, "AI 接口请求失败")
		return
	}
	OK(w, tasks)
...
func DeleteUserCanvasImageTask(w http.ResponseWriter, r *http.Request, id string) {
	...
	if err := service.DeleteUserCanvasImageTask(user.ID, id); err != nil {
		logger.Errorf("delete canvas image task failed: user=%s id=%s err=%v", user.ID, id, err)
		Fail(w, "AI 接口请求失败")
		return
	}
...
func DeleteUserCanvasTasks(w http.ResponseWriter, r *http.Request) {
	...
	if err := service.DeleteUserCanvasTasks(user.ID, request.SourceID, request.NodeIDs); err != nil {
		logger.Errorf("delete canvas tasks failed: user=%s source=%s err=%v", user.ID, request.SourceID, err)
		Fail(w, "AI 接口请求失败")
		return
	}
...
func CreateCanvasAudioTask(w http.ResponseWriter, r *http.Request) {
	...
	body, contentType, endpoint, _, nodeID, sourceID, clientTaskID, prompt, channelID, characterIDs, err := readCanvasTaskAIRequest(r, "/audio/speech")
	if err != nil {
		Fail(w, err.Error())
		return
	}
	...
	channel, resolvedUserChannelID, err := selectAIRequestChannel(user, modelName, channelID, userChannelID)
	if err != nil {
		logger.Errorf("canvas audio task select channel failed: model=%s err=%v", modelName, err)
		failAIChannelSelect(w, err, "AI 接口请求失败")
		return
	}
	...
	if err != nil {
		logger.Errorf("create canvas audio task failed: user=%s err=%v", user.ID, err)
		Fail(w, "AI 接口请求失败")
		return
	}
...
func GetCanvasAudioTask(w http.ResponseWriter, r *http.Request, id string) {
	...
	if err != nil {
		logger.Errorf("read canvas audio task failed: user=%s id=%s err=%v", user.ID, id, err)
		Fail(w, "AI 接口请求失败")
		return
	}
	if !found {
...
func readCanvasTaskAIRequest(r *http.Request, fallbackEndpoint string) ([]byte, string, string, string, string, string, string, string, string, []string, error) {
	...
	if strings.HasPrefix(contentType, "multipart/form-data") {
		...
		charIDs := parseCharacterIDs(meta["_canvas_character_ids"])
		if len(charIDs) > 0 {
			user, ok := service.UserFromContext(r.Context())
			if ok {
		...
		return body, cleanedContentType, endpoint, meta["_canvas_source"], meta["_canvas_node_id"], meta["_canvas_source_id"], meta["_canvas_task_id"], meta["_canvas_prompt"], meta["_canvas_channel_id"], charIDs, nil
	}
	var wrapper struct {
	...
	if len(body) == 0 {
		return nil, "", "", "", "", "", "", "", "", nil, errors.New("任务请求体不能为空")
	}
		endpoint := firstNonEmpty(wrapper.Endpoint, fallbackEndpoint)
		return body, "application/json", endpoint, wrapper.Source, wrapper.NodeID, wrapper.SourceID, firstNonEmpty(wrapper.ClientTaskID, wrapper.TaskID), wrapper.Prompt, wrapper.ChannelID, wrapper.CharacterIDs, nil
	}
...
func injectCharacterAnchor(user model.AuthUser, body []byte, prompt string, modelName string, characterIDs []string) ([]byte, string) {
	...
	if anchor != "" {
		if prompt == "" {
			...
			newPrompt = anchor + prompt
		}
	}
	return body, newPrompt
}

func parseCharacterIDs(raw string) []string {
	if raw == "" {
		return nil
	}
	var ids []string
...
func injectMultipartPrompt(body []byte, anchor string) []byte {
	_, params, err := mime.ParseMediaType("multipart/form-data; boundary=placeholder")
	if err != nil {
		return body
	}
	idx := bytes.Index(body, []byte("form-data; name=\"prompt\""))
	if idx < 0 {
		return body
	}
	// find the value: after \r\n\r\n, until next boundary
	dataStart := bytes.Index(body[idx:], []byte("\r\n\r\n"))
	if dataStart < 0 {
		return body
	}
	dataStart = idx + dataStart + 4
	boundaryEnd := bytes.Index(body[:64], []byte("\r\n"))
	if boundaryEnd < 0 {
		boundaryEnd = bytes.Index(body, []byte("\n"))
	}
	_ = params
	boundary := body[:boundaryEnd]
	if len(boundary) == 0 || boundary[0] != '-' {
		return body
	}
	nextBoundary := bytes.Index(body[dataStart:], boundary)
	var dataEnd int
	if nextBoundary >= 0 {
		dataEnd = dataStart + nextBoundary - 2 // before \r\n
	} else {
		dataEnd = len(body)
	}
	var result bytes.Buffer
	result.Write(body[:dataStart])
	result.WriteString(anchor)
	if dataEnd < len(body) {
		result.Write(body[dataEnd:])
	}
	return result.Bytes()
}
