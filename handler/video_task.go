...
func UserVideoTasks(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		logger.Errorf("list video tasks failed: user=%s err=%v", user.ID, err)
		Fail(w, "AI 接口请求失败")
		return
	}
	OK(w, tasks)
...
func DeleteUserVideoTask(w http.ResponseWriter, r *http.Request, id string) {
	...
	if err := service.DeleteUserVideoTask(user.ID, id); err != nil {
		logger.Errorf("delete video task failed: user=%s id=%s err=%v", user.ID, id, err)
		Fail(w, "AI 接口请求失败")
		return
	}
...
func CancelUserVideoTask(w http.ResponseWriter, r *http.Request, id string) {
	...
	if err := service.CancelUserVideoTask(user.ID, id); err != nil {
		logger.Errorf("cancel video task failed: user=%s id=%s err=%v", user.ID, id, err)
		Fail(w, "AI 接口请求失败")
		return
	}
...
func proxyAIVideoTaskRequest(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		logger.Errorf("AI video request read failed: %v", err)
		Fail(w, "AI 接口请求失败")
		return
	}
	user, ok := service.UserFromContext(r.Context())
	...
	if charIDs := r.Header.Get("X-Character-IDs"); charIDs != "" {
		ids := strings.Split(charIDs, ",")
		for i := range ids {
	...
	if err != nil {
		logger.Errorf("AI video select channel failed: model=%s err=%v", modelName, err)
	...
	if userChannelID == "" {
		...
		if err != nil {
			logger.Errorf("AI video read model cost failed: model=%s err=%v", modelName, err)
			Fail(w, "AI 接口请求失败")
			return
		}
	...
	if err != nil {
		logger.Errorf("AI video normalize request failed: model=%s err=%v", modelName, err)
		Fail(w, "AI 接口请求失败")
		return
	}
	if isViduChannel(channel, modelName) && viduRequestBodyHasImages(body) {
		upstreamPath = "/reference2video"
	}
	request, err := http.NewRequest(http.MethodPost, service.BuildModelChannelURL(channel, upstreamPath), bytes.NewReader(body))
	if err != nil {
		logger.Errorf("AI video build request failed: url=%s err=%v", service.BuildModelChannelURL(channel, upstreamPath), err)
		Fail(w, "AI 接口请求失败")
		return
	}
	request.Header.Set("Authorization", viduAuthHeader(channel)+channel.APIKey)
	if contentType != "" {
	...
	if err != nil {
		logger.Errorf("save video task failed: model=%s err=%v", modelName, err)
		Fail(w, "AI 接口请求失败")
		return
	}
...
func serveAIVideoTask(w http.ResponseWriter, r *http.Request, id string) bool {
	...
	if err != nil {
		logger.Errorf("read video task failed: id=%s user=%s err=%v", id, user.ID, err)
		Fail(w, "AI 接口请求失败")
		return true
	}
	if !found {
...
func pollVideoTaskFromUpstream(task model.VideoTask) (service.VideoTaskPollUpdate, error) {
	...
	if isSiliconflowVideoChannel(channel, task.Model) {
		endpoint = "/video/status"
		method = http.MethodPost
		reqBody, _ := json.Marshal(map[string]any{"requestId": pollID})
		request, err = http.NewRequest(http.MethodPost, service.BuildModelChannelURL(channel, endpoint), bytes.NewReader(reqBody))
		if err == nil {
...
func normalizeVideoCreateBody(body []byte, contentType string, modelName string, channel model.ModelChannel, upstreamPath string) ([]byte, string, error) {
	...
	if isViduChannel(channel, modelName) {
		return normalizeViduVideoBody(body, contentType, modelName, channel)
	}
	if isSiliconflowVideoChannel(channel, modelName) {
		return normalizeSiliconflowVideoBody(body, contentType, modelName, channel)
	}
...
func transformVideoCreatePayload(payload []byte, request *http.Request, channel model.ModelChannel, modelName string) []byte {
	...
	if isViduChannel(channel, modelName) {
		if transformed, ok := transformViduCreateVideoResponse(payload, modelName); ok {
			return transformed
		}
	}
	if isSiliconflowVideoChannel(channel, modelName) {
		if transformed, ok := transformSiliconflowCreateResponse(payload, modelName); ok {
			return transformed
		}
	}
	return payload
...
func transformVideoStatusPayload(payload []byte, request *http.Request, channel model.ModelChannel, modelName string) []byte {
	...
	if isViduChannel(channel, modelName) {
		if transformed, ok := transformViduTaskStatusResponse(payload, modelName); ok {
			return transformed
		}
	}
	if isSiliconflowVideoChannel(channel, modelName) {
		if transformed, ok := transformSiliconflowStatusResponse(payload, modelName); ok {
			return transformed
		}
	}
	return payload
...
func readVideoCreateErrorMessage(raw []byte, transformed []byte, channel model.ModelChannel, modelName string) string {
	...
	if isViduChannel(channel, modelName) {
		return firstNonEmpty(readViduCreateError(raw), readNormalizedVideoError(transformed))
	}
...
func refundVideoCredits(userID string, modelName string, credits int, endpoint string) {
	if err := service.RefundUserCredits(userID, modelName, credits, endpoint); err != nil {
		logger.Errorf("AI video refund credits failed: user=%s model=%s credits=%d err=%v", userID, modelName, credits, err)
	}
}

func CacheVideoTask(w http.ResponseWriter, r *http.Request, taskID string) {
	user, ok := service.UserFromContext(r.Context())
	if !ok {
		Fail(w, "未登录或权限不足")
		return
	...
	if err != nil {
		logger.Errorf("cache video task failed: id=%s err=%v", taskID, err)
		Fail(w, "缓存视频失败")
		return
	}
	OK(w, map[string]any{"cached": true, "cachedPath": cached.CachedPath})
}

func ServeCachedVideo(w http.ResponseWriter, r *http.Request, taskID string) {
	path, ok := service.GetCachedVideoPath(taskID)
	if !ok {
		http.NotFound(w, r)
		return
	}
...
func injectCharacterData(body []byte, contentType string, anchor string, refs []string, seed int64) ([]byte, string) {
	if len(body) == 0 {
	...
	if body[0] == '{' {
		...
		if anchor != "" {
			if existing, ok := m["prompt"].(string); ok {
				m["prompt"] = anchor + existing
			}
		}
		if len(refs) > 0 {
			existing, _ := m["images"].([]any)
