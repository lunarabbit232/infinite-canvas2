...
func proxyAIGetRequest(w http.ResponseWriter, r *http.Request, path string) {
	...
	if err != nil {
		logger.Errorf("AI proxy select channel failed: model=%s err=%v", modelName, err)
...
func proxyAIRequest(w http.ResponseWriter, r *http.Request, path string) {
	...
	if err != nil {
		logger.Errorf("AI proxy request read failed: %v", err)
		Fail(w, "AI 接口请求失败")
		return
	}
	user, ok := service.UserFromContext(r.Context())
	...
	if userChannelID == "" {
		...
		if err != nil {
			logger.Errorf("AI proxy read model cost failed: model=%s err=%v", modelName, err)
			Fail(w, "AI 接口请求失败")
			return
		}
	...
	if isKIEChannel(channel, modelName) && upstreamPath == "/jobs/createTask" {
		...
		if err != nil {
			logger.Errorf("AI proxy normalize KIE request failed: model=%s err=%v", modelName, err)
			Fail(w, "AI 接口请求失败")
			return
		}
		...
		if err != nil {
			logger.Errorf("AI proxy normalize APIMart video request failed: model=%s err=%v", modelName, err)
			Fail(w, "AI 接口请求失败")
			return
		}
		...
		if err != nil {
			logger.Errorf("AI proxy normalize APIMart image request failed: model=%s err=%v", modelName, err)
			Fail(w, "AI 接口请求失败")
			return
		}
	}
	...
	if err != nil {
		logger.Errorf("AI proxy build request failed: url=%s err=%v", service.BuildModelChannelURL(channel, upstreamPath), err)
		Fail(w, "AI 接口请求失败")
		return
	}
	request.Header.Set("Authorization", "Bearer "+channel.APIKey)
	...
	copyAIResponse(w, request, channel, aiLogContext{
		...
		if credits > 0 {
			if err := service.RefundUserCredits(user.ID, modelName, credits, upstreamPath); err != nil {
				logger.Errorf("AI proxy refund credits failed: user=%s model=%s credits=%d err=%v", user.ID, modelName, credits, err)
			}
		}
	})
}
...
func copyAIResponse(w http.ResponseWriter, request *http.Request, channel model.ModelChannel, logContext aiLogContext, onFailure func()) {
	...
	if err != nil {
		logger.Errorf("AI proxy request failed: url=%s err=%v", request.URL.String(), err)
		if onFailure != nil {
			onFailure()
		}
...
func resolveAIProxyPath(channel model.ModelChannel, modelName string, path string) string {
	...
	if isViduChannel(channel, modelName) {
		...
		if strings.HasPrefix(path, "/videos/") && !strings.HasSuffix(path, "/content") {
			taskID := strings.TrimSpace(strings.TrimPrefix(path, "/videos/"))
			if taskID != "" && !strings.Contains(taskID, "/") {
				return "/tasks/" + url.PathEscape(taskID) + "/creations"
			}
		}
		return path
	}
