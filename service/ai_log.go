...
func SaveAICallLog(input AICallLogInput) {
	...
	if err := appendAICallLog(item); err != nil {
		logger.Errorf("write ai call log failed err=%v", err)
	}
}
...
func RefreshAILogCleanupScheduler() {
	...
	if err != nil {
		logger.Errorf("load ai log cleanup setting failed err=%v", err)
		return
	}
	...
	if _, err := aiLogCleanupCron.AddFunc(setting.Cron, func() {
		...
		if err != nil {
			logger.Errorf("scheduled ai log cleanup failed err=%v", err)
			return
		}
		log.Printf("scheduled ai log cleanup done removedFiles=%d retentionDays=%d", removed, setting.RetentionDays)
	}); err != nil {
		logger.Errorf("add ai log cleanup cron failed cron=%s err=%v", setting.Cron, err)
	}
}
...
func LocalDirectAILogEnabled() bool {
	settings, err := repository.GetSettings()
	if err != nil {
		logger.Errorf("load local direct ai log setting failed err=%v", err)
		return false
	}
...
func readAICallLogs() ([]model.AICallLog, error) {
	...
	for _, file := range files {
		...
		if err != nil {
			logger.Errorf("read ai call log file failed file=%s err=%v", file, err)
			continue
		}
