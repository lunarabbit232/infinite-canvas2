...
func MeasureAllEnabledStorageProviders() {
	settings, err := repository.GetSettings()
	if err != nil {
		logger.Errorf("storage capacity settings load failed err=%v", err)
		return
	}
	settings = normalizeSettings(settings)
	storage := settings.Private.Storage
	...
	for i, provider := range storage.Providers {
		...
		bytes, err := measureS3Provider(provider)
		if err != nil {
			logger.Errorf("storage capacity measure failed provider=%s err=%v", provider.Name, err)
			continue
		}
		provider.CapacityBytes = bytes
	...
	if changed {
		settings.Private.Storage = storage
		if _, err := repository.SaveSettings(settings, now()); err != nil {
			logger.Errorf("storage capacity settings save failed err=%v", err)
		}
	}
}
...
func RefreshStorageCapacityScheduler() {
	...
	settings, err := repository.GetSettings()
	if err != nil {
		logger.Errorf("load storage capacity setting failed err=%v", err)
		return
	}
	...
	if _, err := storageCapacityCron.AddFunc(setting.Cron, MeasureAllEnabledStorageProviders); err != nil {
		logger.Errorf("add storage capacity cron failed cron=%s err=%v", setting.Cron, err)
	}
}
