...
func SelectModelChannelForModel(modelName string, channelID string) (model.ModelChannel, error) {
	...
	if strings.TrimSpace(channelID) != "" {
		...
		// channelID 不匹配（例如前端残留的旧渠道），回退到按权重自动选，避免误报「指定模型渠道不可用」。
	}
	total := 0
	for _, channel := range channels {
...
func BuildModelChannelURL(channel model.ModelChannel, path string) string {
	...
	if !strings.HasSuffix(lowerBaseURL, "/v1") && !strings.HasSuffix(lowerBaseURL, "/api/v3") && !strings.HasSuffix(lowerBaseURL, "/api/plan/v3") && !strings.Contains(lowerBaseURL, "/ent/v2") && !strings.Contains(lowerBaseURL, "/ent/v3") {
		baseURL += "/v1"
	}
	return baseURL + path
}
