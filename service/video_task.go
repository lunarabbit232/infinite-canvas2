...
func CancelUserVideoTask(userID string, id string) error {
...
func runVideoTaskPoller() {
	...
	for range videoTaskPollWake {
		for {
			...
			if err != nil {
				logger.Errorf("list due video tasks failed err=%v", err)
			...
			if lastCleanupAt.IsZero() || current.Sub(lastCleanupAt) >= videoTaskCleanupInterval {
				if err := repository.DeleteFinishedVideoTasksBefore(videoTaskTime(current.Add(-videoTaskFinishedRetention))); err != nil {
					logger.Errorf("cleanup finished video tasks failed err=%v", err)
				}
			...
			for _, task := range tasks {
				...
				go func(task model.VideoTask) {
					...
					if err := UpdateVideoTaskFromPoll(task, update); err != nil {
						logger.Errorf("update video task failed id=%s err=%v", task.ID, err)
					}
				}(task)
			}
			waitForNextVideoTaskPoll()
...
const videoCacheDir = "data/videos"

func CacheVideoResult(task model.VideoTask) (model.VideoTask, error) {
	if task.VideoURL == "" || task.CachedPath != "" {
		return task, nil
	}
	resp, err := http.Get(task.VideoURL)
	if err != nil {
		return task, err
	}
	defer resp.Body.Close()
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		return task, errors.New("下载视频失败: HTTP " + resp.Status)
	}
	if err := os.MkdirAll(videoCacheDir, 0o755); err != nil {
		return task, err
	}
	filename := "video-" + task.ID + ".mp4"
	cachedPath := filepath.Join(videoCacheDir, filename)
	...
	f.Close()
	task.CachedPath = cachedPath
	_, err = repository.SaveVideoTask(task)
	return task, err
}

func GetCachedVideoPath(taskID string) (string, bool) {
	task, found, err := repository.GetVideoTask(taskID)
	if err != nil || !found || task.CachedPath == "" {
		return "", false
	}
	if _, statErr := os.Stat(task.CachedPath); statErr != nil {
		return "", false
	}
	return task.CachedPath, true
}
