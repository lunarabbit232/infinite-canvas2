...
// GetPromptByID 根据 ID 查询提示词。
func GetPromptByID(id string) (*model.Prompt, error) {
	db, err := DB()
	if err != nil {
		return nil, err
	}
...
// applyPromptTagsFilter 应用 JSON 标签条件（AND 逻辑：全部选中标签都需命中）。
